...
# Get Mikero Tools
RUN curl "https://mikero.bytex.digital/api/download?filename=depbo-tools-0.10.18-linux-amd64.tgz" -o depbo-tools-0.10.18-linux-amd64.tgz
RUN tar -xzf depbo-tools-0.10.18-linux-amd64.tgz
RUN mv depbo-tools-0.10.18/bin/* /usr/bin/
RUN mv depbo-tools-0.10.18/lib/* /usr/lib/
RUN rm -rf depbo-tools-0.10.18-linux-amd64.tgz
RUN rm -rf depbo-tools-0.10.18
